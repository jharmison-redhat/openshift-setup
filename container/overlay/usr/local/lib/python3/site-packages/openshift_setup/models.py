...
class Namespace(BaseModel):
    ...
    def render(self, cluster_dir: Path | None = None) -> None:
        ...
        app = Application(
            ...
            values=values,
            rewrite={"metadata": {"name": f"{self.name}-namespace"}},
        )
        logger.debug(app)
...
class RawApplication(BaseModel):
    """A representation of a raw ArgoCD application to populate"""

    name: str
    namespace: str
    createNamespace: bool = False
    project: str = "default"
    sync_wave: Optional[int]
    source: Dict[str, Any] = {}
    values: Dict[str, Any] = {}
    secrets: Dict[str, Any] = {}

    def load_template(self, value_files: List[str] = []) -> Dict[str, Any]:
        template = dict(
        ...
        if self.createNamespace:
            create_namespace = dict(spec=dict(syncPolicy=dict(syncOptions=["CreateNamespace=true"])))
            template = merge(template, create_namespace)

        if len(value_files) > 0:
            value_merge = dict(helm=dict(valueFiles=value_files))
            extra_source = dict(
                repoURL=vars.get("ARGO_GIT_URL", ""),
                targetRevision=vars.get("ARGO_GIT_REVISION", ""),
                ref="values",
            )
            add_source = dict(
                spec=dict(
                    sources=[
                        merge(self.source, value_merge),
                        extra_source,
                    ]
                )
            )
        else:
            add_source = dict(spec=dict(sources=[self.source]))

        template = merge(template, add_source)

        if self.sync_wave is not None:
            sync_wave = dict(metadata=dict(aanotations={"argocd.argoproj.io/sync-wave": str(self.sync_wave)}))
            template = merge(template, sync_wave)
    ...
    def render(self, cluster_dir: Path | None = None) -> None:
        ...
        if self.values:
            ...
            values_template = Template(yaml.dump(new, explicit_start=True))
            values_data = values_template.safe_substitute(**subs)
        ...
        if self.secrets:
            ...
            secrets_template = Template(yaml.dump(new, explicit_start=True))
            secrets_data = secrets_template.safe_substitute(**subs)
        ...
        if values_yaml.exists():
            ...
            value_files.append(f"$values/{values_dir}/values.yaml")
        ...
        if secrets_enc_yaml.exists():
            ...
            value_files.append(f"$values/{values_dir}/secrets.enc.yaml")

        app = self.load_template(value_files)
    ...
    def render(self, cluster_dir: Path | None = None) -> None:
        ...
        if not self.is_application_set:
            ...
            if secrets_enc_yaml.exists():
                logger.debug(f"Adding secrets.enc.yaml values to {self.name}")
                value_files.append(
                    f"secrets+age-import:///helm-secrets-private-keys/argo.txt?../../{values_dir}/secrets.enc.yaml"
                )

            # If we identified any values to track, update the application to use them
            if value_files:
                self.rewrite = merge(self.rewrite, {"spec": {"source": {"helm": {"valueFiles": value_files}}}})
        else:
...
class Demo(BaseModel):
    ...
    rawApplications: List[RawApplication] = []
    ...
    def render(self, cluster_dir: Path | None = None) -> None:
        ...
        for application in self.applications:
            ...
            application.render()
        for application in self.rawApplications:
            logger.debug(f"Rendering {application.name} application")
            application.render()
