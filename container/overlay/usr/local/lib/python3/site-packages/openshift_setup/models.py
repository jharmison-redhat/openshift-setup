...
class Application(BaseModel):
    ...
    @classmethod
    def _diff(cls, old: dict, new: dict) -> dict:
        diff = {}
        for k, v_old in old.items():
            if k not in new:
                diff[k] = {'old': v_old, 'new': None}
            else:
                v_new = new[k]
                if isinstance(v_old, dict) and isinstance(v_new, dict):
                    nested = cls._diff(v_old, v_new)
                    if nested:
                        diff[k] = nested
                elif v_old != v_new:
    ...
    def render(self, cluster_dir: Path | None = None) -> None:
        ...
        if self.values:
            ...
            if values_yaml.exists():
                ...
                logger.debug(f"Found diff for values of {self.name}: {diff}")
                if diff:
                    update = True
            else:
                new = self.values
            ...
            if update:
                logger.debug(f"Writing out values {self.values} for {self.name}")
                with open(values_yaml, "w") as f:
                    yaml.dump(new, f, explicit_start=True)
            else:
                logger.debug(f"Skipping update of {self.name} values because no diff")
        ...
        if self.secrets:
            ...
            if secrets_yaml.exists():
                ...
                diff = self._diff(old, new)
                logger.debug(f"Found diff for secrets of {self.name}: {diff}")
                if diff:
                    update = True
            else:
                new = self.secrets
                update = True
            if update:
                logger.debug(f"Writing out secrets {self.secrets} for {self.name}")
                with open(secrets_yaml, "w") as f:
                    yaml.dump(new, f, explicit_start=True)
            else:
                logger.debug(f"Skipping update of {self.name} secrets because no diff")
