...
def shell(cmd: str = "", fail: bool = True, stderr: int | None = subprocess.STDOUT) -> Iterable[str]:
...
def shellw(cmd: str = "", fail: bool = True, stderr: int | None = subprocess.STDOUT) -> str:
    """Run a command in a subprocess, wait for it to finish, return all
    lines from it as a single string."""
    return "\n".join([line for line in shell(cmd=cmd, fail=fail, stderr=stderr)]    )
